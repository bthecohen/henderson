...
(defn right-split [painter! n]
  (if (= n 0)
      ...
      (let [smaller (right-split painter! (- n 1))]
        (beside painter! (above smaller smaller 0.66666) 0.66666))))

(defn down-split [painter! n]
  (if (= n 0)
      painter!
      (let [smaller (down-split painter! (- n 1))]
        (above painter! (beside smaller smaller 0.66666) 0.66666))))
...
(def star-of-david [{:start [0.5 0] :end [0 0.75] :color "blue"}
                    {:start [0 0.75] :end [1 0.75] :color "blue"}
                    {:start [1 0.75] :end [0.5 0] :color "blue"}
                    {:start [0.5 1] :end [0 0.25] :color "blue"}
                    {:start [0 0.25] :end [1 0.25] :color "blue"}
                    {:start [1 0.25] :end [0.5 1] :color "blue"}])
(def x-segments [{:start [0 0] :end [1 1] :color "black"}
                 {:start [0 1] :end [1 0] :color "black"}
                 {:start [0.5 0] :end [0.5 1] :color "black"}
                 {:start [0 0.5] :end [1 0.5] :color "black"}])
...
((square-limit painter-combined! 3) unit-frame)
